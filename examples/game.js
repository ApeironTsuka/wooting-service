...
let kb = new wootingClient(), layer;
kb.connect();
kb.on('ready', ready);
...
function printNum(n, row) {
  ...
  r = n;
  for (let i = o; i < y + o; i++) { layer.setLoc(row, i, 255, 255, 0); } o+=y;
  for (let i = o; i < b + o; i++) { layer.setLoc(row, i, 0, 0, 255); } o+=b;
  for (let i = o; i < g + o; i++) { layer.setLoc(row, i, 0, 255, 0); } o+=g;
  for (let i = o; i < r + o; i++) { layer.setLoc(row, i, 255, 0, 0); }
}
class ship {
  ...
  draw() { layer.setLoc(Math.floor(this.y), Math.floor(this.x), ...this.color()); }
}
class bullet {
  ...
  draw() {
    ...
    if ((this != bullets[0]) && (f(bullets[0].x) == f(x)) && (f(bullets[0].y) == f(y))) {
      ...
      layer.setLoc(f(y), f(x), (c1[0] + c2[0]) & 0xff, (c1[1] + c2[1]) & 0xff, (c1[2] + c2[2]) & 0xff);
    } else { layer.setLoc(f(y), f(x), ...this.color); }
  }
}
...
function ready() {
  kb.watchAnalog();
  kb.registerOwnLayer('game').then((uid) => {
    layer = new Layer(kb, uid);
    begin();
  });
}
function begin() {
  ...
  tmr = setInterval(() => {
    if (over) {
      for (let y = 2; y < 5; y++) { for (let x = 1; x < 11; x++) { layer.setLoc(y, x, 0, 0, 0); } }
      layer.setKey(kb.LEDs.G, 0, 255, 0);
      layer.setKey(kb.LEDs.A, 0, 255, 0);
      layer.setKey(kb.LEDs.M, 0, 255, 0);
      layer.setKey(kb.LEDs.O, 255, 0, 0);
      layer.setKey(kb.LEDs.V, 255, 0, 0);
      layer.setKey(kb.LEDs.E, 255, 255, 0);
      layer.setKey(kb.LEDs.R, 255, 0, 0);
      kb.updateOwnLayer(layer);
      clearInterval(tmr);
      return;
    }
    for (let y = 1; y < 5; y++) { for (let x = 1; x < 11; x++) { layer.setLoc(y, x, 50, 50, 50); } }
    layer.setLoc(1, 0, 50, 50, 50); layer.setLoc(1, 11, 50, 50, 50); layer.setLoc(1, 12, 50, 50, 50);
    layer.setLoc(4, 11, 50, 50, 50);
    if (kb.isISO) { layer.resetLoc(4, 1); }
    ...
    if (kb.readKey(kb.Analog.Up) > kb.actuationPoint) { player.up(); }
    if (kb.readKey(kb.Analog.Down) > kb.actuationPoint) { player.down(); }
    if (kb.readKey(kb.Analog.Right) > kb.actuationPoint) {
      let b = new bullet();
      b.x = player.x + 1;
      b.y = player.y;
      b.dir = 0.5;
