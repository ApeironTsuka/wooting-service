...
class wootingClient extends wootingConnection {
  ...
  connect(port = DEFAULT_PORT) {
    ...
    this.on('ipc_ownLayerChanged', ({ id, z, enabled }) => this.emit('ownLayerChanged', id, z, enabled));
    this.on('ipc_layerChanged', ({ uid, event, name, description, z, enabled }) => {
      switch (event) {
        case 'add': this.emit('layerAdd', uid, name, description, z); break;
        case 'rem': this.emit('layerRem', uid); break;
        case 'move': this.emit('layerMoved', uid, z); break;
        case 'enable': this.emit('layerEnabled', uid, enabled); break;
      }
    });
    ...
    this.on('ipc_keyboardChanged', ({ firmware, model, isANSI, rows, cols, keyCount, profile, isTwo, actuationPoint }) => {
  ...
  updateOwnLayer(layer) {
    if (!this.connected) { return Promise.reject(dcError()); }
    return new Promise((resolve, reject) => this.send('updateOwnLayer', { uid: layer.uid, map: layer.map }, resolve));
  }
  watchAnalog() {
    if (!this.connected) { return Promise.reject(dcError()); }
  ...
  watchLayers() {
    if (!this.connected) { return Promise.reject(dcError()); }
    return new Promise((resolve, reject) => this.send('watchLayers', undefined, resolve));
  }
  unwatchLayers() {
    if (!this.connected) { return Promise.reject(dcError()); }
    return new Promise((resolve, reject) => this.send('unwatchLayers', undefined, resolve));
  }
  disableOwnLayer(layer) {
    if (!this.connected) { return Promise.reject(dcError()); }
    return new Promise((resolve, reject) => this.send('layerEnabled', { uid: `${this.id}-${layer.uid}`, v: false }, resolve));
  }
  enableOwnLayer(layer) {
    if (!this.connected) { return Promise.reject(dcError()); }
    return new Promise((resolve, reject) => this.send('layerEnabled', { uid: `${this.id}-${layer.uid}`, v: true }, resolve));
  }
  moveOwnLayer(layer, z) {
    if (!this.connected) { return Promise.reject(dcError()); }
    return new Promise((resolve, reject) => this.send('moveLayer', { uid: `${this.id}-${layer.uid}`, z }, resolve));
  }
  disableLayer(uid) {
    if (!this.connected) { return Promise.reject(dcError()); }
    return new Promise((resolve, reject) => this.send('layerEnabled', { uid, v: false }, resolve));
  }
  enableLayer(uid) {
    if (!this.connected) { return Promise.reject(dcError()); }
    return new Promise((resolve, reject) => this.send('layerEnabled', { uid, v: true }, resolve));
  }
  moveLayer(uid, z) {
    if (!this.connected) { return Promise.reject(dcError()); }
  ...
  takeControl() {
    if (!this.connected) { return Promise.reject(dcError()); }
    return new Promise((resolve, reject) => this.send('control', { v: true }, resolve));
  }
  releaseControl() {
    if (!this.connected) { return Promise.reject(dcError()); }
    return new Promise((resolve, reject) => this.send('control', { v: false }, resolve));
  }
...
class wootingServerClient extends wootingConnection {
  ...
  keyboardChanged({ firmware, model, isANSI, rows, cols, keyCount, profile, isTwo, actuationPoint }) { this.send('keyboardChanged', { firmware, model, isANSI, rows, cols, keyCount, profile, isTwo, actuationPoint }); }
  ownLayerChanged(id, z, enabled) { this.send('ownLayerChanged', { id, z, enabled }); }
  layerChanged({ uid, event, name, description, z, enabled }) { this.send('layerChanged', { uid, event, name, description, z, enabled }); }
